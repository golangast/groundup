...
func GenServer(p string, f string, s string, g string) {
	...
	err := Makefolder(p)
	ErrorCheck(err)
	err = Makefolder(p + "/templates")
	ErrorCheck(err)
	//make folder for handler
	err = Makefolder("../app/handler/get/" + datavar)
	ErrorCheck(err)
	...
	/* write to files*/
	err = Writetemplate(Servertemp, sfile, nil)
	ErrorCheck(err)
	err = Writetemplate(Headertemp, hfile, m)
	ErrorCheck(err)
	err = Writetemplate(Footertemp, hfile, mf)
	ErrorCheck(err)
	err = Writetemplate(Bodytemp, bfile, mb)
	ErrorCheck(err)
