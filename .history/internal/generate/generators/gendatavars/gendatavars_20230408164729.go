...
func Gendatavars(p, urls, datavar string) {
	...
	//make route file
	routefile, err := Makefile("handler/get/" + datavar + "/" + datavar + ".go")
	if err != nil {
	...
	found, err := genutility.FindText("../app/app.go", `func `+strings.ToUpper(urls))
	if err != nil {
		fmt.Print(err)
	}
	if !found {
		err := genutility.UpdateText("../app/app.go", "//#handler", `func `+strings.ToUpper(urls)+`(c echo.Context) error {
